...
const htmlPluginEntries = templateFiles.map((template) => new HTMLWebpackPlugin({
	...
	chunks: [ template.input.slice(0, -5) === 'index' ? 'home' : template.input.slice(0, -5) ],
}));

module.exports = {
  entry: {
		home: path.resolve(environment.paths.source, 'js', 'home.js'),
		countries: path.resolve(environment.paths.source, 'js', 'countries.js'),
	},
  ...
  plugins: [
    ...
    new CopyWebpackPlugin({
      patterns: [
        ...
        {
          from: path.resolve(environment.paths.source, 'icomoon'),
          to: path.resolve(environment.paths.output, 'icomoon'),
          toType: 'dir',
          globOptions: {
            ignore: ['*.DS_Store', 'Thumbs.db'],
          },
